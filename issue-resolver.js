...
const MODEL = MODEL_INFO.gpt3_5_turbo // Use the cheaper model by default
const TOKEN_ENCODING = encodingForModel(MODEL.name);
...
class IssueResolver {
    ...
    constructPrompt(file, messages, includeImplementationFile = true){
        ...
        let constructedPrompt = [promptIntro, promptWarnings, promptImplementation, promptOutro].join("\n\n")
        const tokens = TOKEN_ENCODING.encode(constructedPrompt).length
        
        if(tokens > MODEL.maxTokens && includeImplementationFile){
            console.warn(`😬 Prompt length ${tokens} exceeds ${MODEL.maxTokens} tokens. Proceeding without the implementation file ...`)
            return this.constructPrompt(file, messages, false)
        }
        
        console.log(`🪙 Prompt length: ${tokens} tokens`)
    ...
    async getResolvedFile(file){
        ...
        try {
            ...
            if(error.code === "context_length_exceeded"){
                console.error("❌ Context length exceeded. Consider splitting the file into multiple files.")
                return null
            }
